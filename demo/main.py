...
Features:
  ...
  - Creates new tickets OR updates existing ones (summary + description + custom fields)
...
from clients.jira import JiraClient  # noqa: E402
...
def _phase0_validate(snyk: SnykClient, jira: JiraClient, teams: TeamsClient) -> None:
    ...
    checks = [
        ...
        ("Jira custom fields",  lambda: _check_jira_fields(jira)),
...
def _check_jira_fields(jira: JiraClient) -> None:
    missing = jira.validate_custom_fields()
    if missing:
        raise RuntimeError(f"Missing Jira custom fields: {missing}")
...
def _phase1_collect(snyk: SnykClient, state: DemoState) -> list[SnykTarget]:
    ...
    all_targets, _ = snyk.get_aggregated_targets()
    
    lookup = {t.display_name.lower(): t for t in all_targets}
    targets = [lookup[name.lower()] for name in _DEMO_TARGET_NAMES if name.lower() in lookup]

    found_names = {t.display_name for t in targets}
    for name in _DEMO_TARGET_NAMES:
        if name.lower() not in {n.lower() for n in found_names}:
            logger.warning("  ⚠ Not found in Snyk: %s", name)

    for target in targets:
        logger.info("  ✓ Found: %s (C%d H%d)", target.display_name, target.critical, target.high)
    ...
    state.set_targets(state_targets)
...
def _phase2_sync(
    ...
    """Create or update tickets. Results stored in state."""
    logger.info("=" * 80)
    logger.info("PHASE 2 — Jira Sync (Create or Update)")
    logger.info("=" * 80)

    state.clear_ticket_lists()  # Fresh lists for this run
    ...
    for target in targets:
        try:
            ...
            if ticket is None:
                logger.info("  [%s] No ticket found → trying fallback search", target.display_name)
                ticket = jira.find_open_ticket_by_name(target.display_name)
                if ticket is not None:
                    logger.info("    → Fallback found %s, backfilling snyk_project_id", ticket.key)
            ...
            if ticket is None:
                ...
                url = jira.ticket_url(key)
                logger.info("  ✓ Created %s", key)
                state.add_new_ticket(target.display_name, key, url)
            else:
                ...
                url = jira.ticket_url(ticket.key)
                logger.info("  ✓ Updated %s", ticket.key)
                state.add_updated_ticket(target.display_name, ticket.key, url)

        except Exception as exc:
            logger.error("  ✗ [%s] Error: %s", target.display_name, exc, exc_info=True)

    new_count = len(state.get_new_tickets())
    updated_count = len(state.get_updated_tickets())
    logger.info("Phase 2 — created=%d updated=%d\n", new_count, updated_count)


def _phase3_notify(teams: TeamsClient, state: DemoState, repo_count: int) -> None:
    logger.info("=" * 80)
    logger.info("PHASE 3 — Teams Notification")
    logger.info("=" * 80)

    new_tickets = state.get_new_tickets()
    updated_tickets = state.get_updated_tickets()

    if not new_tickets and not updated_tickets:
        logger.info("  No tickets to report → skipping Teams notification\n")
        return

    card = _build_demo_card(new_tickets, updated_tickets, repo_count)
    ...
    try:
        ...
        if start_phase <= 2 <= end_phase:
            if not targets and not args.load:
                ...
                return 1
            _phase2_sync(targets, jira, state, today)
            state.save()

        # Phase 3: Send Teams notification
        if start_phase <= 3 <= end_phase:
            _phase3_notify(teams, state, len(state.get_targets()))
            state.save()
